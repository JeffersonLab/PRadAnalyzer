! Radiative cross section calculation from MERADGEN
! Source Code: MERADGEN 1.0
! Reference: A. Afanasev, E. Chudakov, A. Ilyichev, V. Zykunov,
!            Comput. Phys. Commun. 176, 218 (2007)

! initialize the constants
! input: beam energy in lab frame (MeV)
     ...
     &bind(C, name = "merad_init")
      ...
      data pi/3.14159265359/, alfa/7.297352568D-3/,
     .m/0.510998918/, m2/0.2611198942/

      En = elab
      s = 2.d0*(En*m + m2)
      als = s*(s - 4.*m2)
      coeb = 4d0*pi*alfa**2*(s - 2d0*m2)/als
      coer = alfa**3*(s - 2d0*m2)/als/pi/4d0

      end

! calculate some kinematic varaibles
! used in fsir when ikey = 1
	...
	return
	 ...
	 double precision function sig(t,pl)
	 implicit none
...
!     . m2*t-8.0*(2.0*m2-t)*m2**2*t))/als
	...
	return
      ...
      double precision function fsir(t,t1,v,z,sig0,nn,ikey)
     &bind(C, name = "merad_fsir")
      use, intrinsic :: ISO_C_BINDING
      implicit none
      real(C_DOUBLE), intent(IN), VALUE :: t, t1, v, z, sig0
      integer(C_INT), intent(IN), VALUE :: ikey
      integer(C_INT), intent(OUT) :: nn
! The cross section of real photon emission
! t,t1,v,z are kinematic invariant
! sig0 is the Born cross section, used for ikey = -1
...
! radiative cross section integrated over z and t1 ds/dt/dv
      real*8 u,uu,vv,v1,vv1,z1,z2,zz,zz1,zz2,ds
      real*8 pl,dz,dz1,vt,pls,tt,tt1,dt,sd,fir,sig
...
!       PRINT *, v, u, ((v-u)**2-4d0*m2*u),
!     . (v-u+2d0*m2+sqrt((v-u)**2-4d0*m2*u))
