...
void SavePixels(const char *path, const char *save_path)
{
    ...
    while(dst_parser.Read())
    {
        if(dst_parser.EventType() == PRadDSTParser::Type::event) {
            ...
            sys.GetReconstructor()->CollectHits(sys.GetDetector());

            Nhits = sys.GetReconstructor()->GetHits().size();
            for(unsigned int i = 0; i < Nhits; ++i)
            {
                id[i] = sys.GetReconstructor()->GetHits().at(i).id;
                E[i] = sys.GetReconstructor()->GetHits().at(i).energy;
            }
